...
uint8_t xmodem_receive_init()
{
  ...
  if (NULL != callback_is_inbound_empty &&
      NULL != callback_is_outbound_full &&
      NULL != callback_set_buffer       &&
      NULL != callback_read_data        && 
...
{
  callback_is_inbound_empty = NULL;
  callback_is_outbound_full = NULL;
  callback_read_data = NULL;
  callback_write_data = NULL;
  receive_state = XMODEM_RECEIVE_UNKNOWN;
  payload_buffer_position = NULL;
  payload_buffer = NULL;
  inbound = NULL;
  returned_size = NULL;
...
void xmodem_receive_set_callback_write(uint8_t(*callback)
...
void xmodem_receive_set_callback_read(uint8_t(*callback)
			(const uint32_t requested_size,
